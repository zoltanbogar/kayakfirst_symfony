...
function generateLegend () {
	...
	for (var dataType in titles) {
		if (dataType != 'currant_distance') {
			inputStatus = dataType.match(/t_1000/) ? 'checked' : '';
			legendHtml[dataOrder.indexOf(dataType) + 1] = 
			'<div class="col-xs-2"><input type="checkbox" class="checkbox-option chart-option' 
			+ '" id="' + dataType + '-graph-toggle" value="in process"><label for="' 
			+ dataType + '-graph-toggle' + '" style="border-color:' + lineColors[dataType] 
			+ '"' + inputStatus +'><span class="label">' + titles[dataType] 
			+ '</span><span class="value" id="tooltip-' + dataType + '">' 
			+'not yet' + '</span></label></div>';
		}
	}
	legendHtml.push('</div>');
	legendHtml.join("");
